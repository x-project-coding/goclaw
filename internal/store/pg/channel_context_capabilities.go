...
func (s *PGMCPServerStore) UpsertContextGrant(ctx context.Context, g *store.MCPContextGrant) error {
...
func (s *PGMCPServerStore) DeleteContextGrant(ctx context.Context, channelInstanceID uuid.UUID, scopeType, scopeKey string, serverID uuid.UUID) error {
	_, err := s.db.ExecContext(ctx,
		`DELETE FROM mcp_context_grants
		 WHERE tenant_id = $1 AND channel_instance_id = $2 AND scope_type = $3 AND scope_key = $4 AND server_id = $5`,
		tenantIDForInsert(ctx), channelInstanceID, scopeType, scopeKey, serverID,
	)
	return err
}

func (s *PGMCPServerStore) ListContextGrants(ctx context.Context, channelInstanceID uuid.UUID, scopeType, scopeKey string) ([]store.MCPContextGrant, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, channel_instance_id, scope_type, scope_key, server_id, enabled,
		        COALESCE(tool_allow, 'null'::jsonb), COALESCE(tool_deny, 'null'::jsonb),
		        COALESCE(config_overrides, 'null'::jsonb), granted_by, created_at, updated_at
		 FROM mcp_context_grants
		 WHERE tenant_id = $1 AND channel_instance_id = $2 AND scope_type = $3 AND scope_key = $4
		 ORDER BY created_at`,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&g.ID, &g.ChannelInstanceID, &g.ScopeType, &g.ScopeKey, &g.ServerID, &g.Enabled,
			&g.ToolAllow, &g.ToolDeny, &g.ConfigOverrides, &g.GrantedBy, &g.CreatedAt, &g.UpdatedAt); err != nil {
			return nil, err
		}
...
func (s *PGMCPServerStore) ListContextGrantsForScope(ctx context.Context, scope store.ChannelContextScope) ([]store.MCPContextGrant, error) {
...
func (s *PGMCPServerStore) SetContextCredentials(ctx context.Context, creds *store.MCPContextCredentials) error {
...
func (s *PGMCPServerStore) GetContextCredentials(ctx context.Context, channelInstanceID uuid.UUID, scopeType, scopeKey string, serverID uuid.UUID) (*store.MCPContextCredentials, error) {
...
func (s *PGMCPServerStore) GetContextCredentialsForScope(ctx context.Context, scope store.ChannelContextScope, serverID uuid.UUID) (*store.MCPContextCredentials, error) {
	id, err := resolveContextChannelInstanceID(ctx, s.db, scope)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	}
...
func (s *PGMCPServerStore) DeleteContextCredentials(ctx context.Context, channelInstanceID uuid.UUID, scopeType, scopeKey string, serverID uuid.UUID) error {
	_, err := s.db.ExecContext(ctx,
...
func (s *PGMCPServerStore) ListContextCredentials(ctx context.Context, channelInstanceID uuid.UUID, scopeType, scopeKey string) ([]store.MCPContextCredentials, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, channel_instance_id, scope_type, scope_key, server_id, api_key, headers, env, created_by, created_at, updated_at
		 FROM mcp_context_credentials
		 WHERE tenant_id = $1 AND channel_instance_id = $2 AND scope_type = $3 AND scope_key = $4
...
func (s *PGMCPServerStore) ListContextCredentialsForScope(ctx context.Context, scope store.ChannelContextScope) ([]store.MCPContextCredentials, error) {
	id, err := resolveContextChannelInstanceID(ctx, s.db, scope)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	}
...
func (s *PGMCPServerStore) scanContextCredentials(row contextCredentialScanner) (*store.MCPContextCredentials, error) {
	...
	if err := row.Scan(&creds.ID, &creds.ChannelInstanceID, &creds.ScopeType, &creds.ScopeKey, &creds.ServerID,
		&apiKey, &headersEnc, &envEnc, &creds.CreatedBy, &creds.CreatedAt, &creds.UpdatedAt); err != nil {
		return nil, err
	}
	...
	if len(headersEnc) > 0 {
		_ = json.Unmarshal(s.decryptJSONB(headersEnc), &creds.Headers)
	}
	if len(envEnc) > 0 {
		_ = json.Unmarshal(s.decryptJSONB(envEnc), &creds.Env)
	}
	return &creds, nil
}

func (s *PGSecureCLIStore) UpsertContextGrant(ctx context.Context, g *store.SecureCLIContextGrant) error {
	if err := validateChannelScope(g.ScopeType, g.ScopeKey); err != nil {
		return err
	}
	if err := store.ValidateUserID(g.GrantedBy); err != nil {
...
func (s *PGSecureCLIStore) DeleteContextGrant(ctx context.Context, channelInstanceID uuid.UUID, scopeType, scopeKey string, binaryID uuid.UUID) error {
	_, err := s.db.ExecContext(ctx,
		`DELETE FROM secure_cli_context_grants
		 WHERE tenant_id = $1 AND channel_instance_id = $2 AND scope_type = $3 AND scope_key = $4 AND binary_id = $5`,
		tenantIDForInsert(ctx), channelInstanceID, scopeType, scopeKey, binaryID,
	)
	return err
}

func (s *PGSecureCLIStore) ListContextGrants(ctx context.Context, channelInstanceID uuid.UUID, scopeType, scopeKey string) ([]store.SecureCLIContextGrant, error) {
	rows, err := s.db.QueryContext(ctx,
...
func (s *PGSecureCLIStore) ListContextGrantsForScope(ctx context.Context, scope store.ChannelContextScope) ([]store.SecureCLIContextGrant, error) {
	id, err := resolveContextChannelInstanceID(ctx, s.db, scope)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	}
...
func (s *PGSecureCLIStore) scanSecureCLIContextGrant(row contextCredentialScanner) (*store.SecureCLIContextGrant, error) {
...
func (s *PGSecureCLIStore) SetContextCredentials(ctx context.Context, creds *store.SecureCLIContextCredentials) error {
	if err := validateChannelScope(creds.ScopeType, creds.ScopeKey); err != nil {
		return err
	}
	if err := store.ValidateUserID(creds.CreatedBy); err != nil {
...
func (s *PGSecureCLIStore) GetContextCredentials(ctx context.Context, channelInstanceID uuid.UUID, scopeType, scopeKey string, binaryID uuid.UUID) (*store.SecureCLIContextCredentials, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, channel_instance_id, scope_type, scope_key, binary_id, encrypted_env,
		        COALESCE(metadata, '{}'::jsonb), credential_type, host_scope, created_by, created_at, updated_at
...
func (s *PGSecureCLIStore) GetContextCredentialsForScope(ctx context.Context, scope store.ChannelContextScope, binaryID uuid.UUID) (*store.SecureCLIContextCredentials, error) {
	id, err := resolveContextChannelInstanceID(ctx, s.db, scope)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	}
...
func (s *PGSecureCLIStore) DeleteContextCredentials(ctx context.Context, channelInstanceID uuid.UUID, scopeType, scopeKey string, binaryID uuid.UUID) error {
	_, err := s.db.ExecContext(ctx,
...
func (s *PGSecureCLIStore) ListContextCredentials(ctx context.Context, channelInstanceID uuid.UUID, scopeType, scopeKey string) ([]store.SecureCLIContextCredentials, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, channel_instance_id, scope_type, scope_key, binary_id, encrypted_env,
		        COALESCE(metadata, '{}'::jsonb), credential_type, host_scope, created_by, created_at, updated_at
		 FROM secure_cli_context_credentials
...
func (s *PGSecureCLIStore) ListContextCredentialsForScope(ctx context.Context, scope store.ChannelContextScope) ([]store.SecureCLIContextCredentials, error) {
	id, err := resolveContextChannelInstanceID(ctx, s.db, scope)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	}
...
func (s *PGSecureCLIStore) scanSecureCLIContextCredentials(row contextCredentialScanner) (*store.SecureCLIContextCredentials, error) {
	...
	if err := row.Scan(&creds.ID, &creds.ChannelInstanceID, &creds.ScopeType, &creds.ScopeKey, &creds.BinaryID,
		...
		&creds.CreatedBy, &creds.CreatedAt, &creds.UpdatedAt); err != nil {
		return nil, err
	}
	if len(creds.EncryptedEnv) > 0 && s.encKey != "" {
