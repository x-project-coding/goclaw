...
func (h *AgentsHandler) handleRegenerate(w http.ResponseWriter, r *http.Request) {
	...
	go h.summoner.RegenerateAgent(id, store.MasterTenantID, ag.Provider, ag.Model, req.Prompt)
...
func (h *AgentsHandler) handleResummon(w http.ResponseWriter, r *http.Request) {
	...
	go h.summoner.SummonAgent(id, store.MasterTenantID, ag.Provider, ag.Model, description)
...
func (h *AgentsHandler) handleCancelSummon(w http.ResponseWriter, r *http.Request) {
	...
	if h.msgBus != nil {
		bus.BroadcastForTenant(h.msgBus, protocol.EventAgentSummoning, store.MasterTenantID, map[string]any{
			"agent_id": id.String(),
			"type":     "failed",
