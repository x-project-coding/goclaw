...
### Patch 20 — `feat(skill-catalog): expand to 106 ops / 22 skills`

- **Base upstream commit:** `3e903778` (rescue merge, PR #49 → dev)
- **Files:** `internal/skillcatalog/catalog.json` — regenerated by x-api `npm run catalog:dump`
  (x-api PR #507) with 7 more namespaces: manage-calls (reads only — proxy), manage-workflows,
  manage-reports, manage-operations, manage-seo, manage-domains, design. 106 ops / 22 skills
  (was ~55). Money (purchases), destructive (DELETEs), and the manage-operations 501 stubs are
  deliberately excluded → those stay `skill raw`. Per-skill gating keeps each agent's tool
  description scoped to its own skills.
- **Why:** the remaining 12 curl-using skills are being migrated to the typed surface (x-skills
...
- **Files:**
  - `internal/store/session_store.go` — adds `ManagedBy string` to `SessionListOpts`
    (optional filter on `metadata->>'managedBy'`, the ops-lead delegation owner).
  - `internal/store/pg/sessions_list.go` — `buildSessionFilter` appends
    `metadata->>'managedBy' = $N` when `opts.ManagedBy != ""` (JSON key is a hardcoded
    literal; only the value is parameterized). Composes as an additional AND — narrows
    within the existing tenant/user_id scope, never widens it.
...
### Patch 28 — `fix(gateway): union media across debounced chat-send merges`

> Numbering note: this entry is authored on `main` (whose registry ends at
> Patch 15) but takes the next free number after `origin/dev`'s registry
...
- **Why:** 42bucks x-ui allows attachment-only chat sends (empty text). Such
  ...
  the primary chat path. Upstream has the same latent bug but no
  attachment-only sender, so pressure to fix it here first.
- **Recovery grep:**
  ```
  grep -nE 'mergedMedia|debounce-media-union' internal/gateway/methods/chat_debounce.go
  grep -nE 'UnionsMediaAcrossItems|LegacyFormatHasNoPhantomItem' \
