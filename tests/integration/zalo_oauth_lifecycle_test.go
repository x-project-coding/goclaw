...
func TestZaloOAuthLifecycle(t *testing.T) {
	...
	creds, err := zalooauth.LoadCreds(got.Credentials)
	if err != nil {
	...
	creds.OAID = "oa-int-1"
	if err := zalooauth.Persist(ctx, ciStore, inst.ID, creds); err != nil {
		t.Fatalf("Persist: %v", err)
	}
	// Read back again — verify Update wrote and Get decrypted.
	got2, _ := ciStore.Get(ctx, inst.ID)
	creds2, _ := zalooauth.LoadCreds(got2.Credentials)
...
func (m *mockZaloServer) Override(ch *zalooauth.Channel) {
